...
impl GitignoreRules {
    fn new() -> Self {
        ...
        rules.insert(
            "kotlin".to_string(),
            vec!["/build/"].into_iter().map(String::from).collect(),
        );
        rules.insert(
            "swift".to_string(),
        ...
        rules.insert(
            "php".to_string(),
            vec!["vendor/"].into_iter().map(String::from).collect(),
        );
        rules.insert(
            "react".to_string(),
        ...
        rules.insert(
            "asp.net".to_string(),
            vec!["bin/", "obj/"].into_iter().map(String::from).collect(),
        );
        rules.insert(
            "unity".to_string(),
            vec![
        ...
        rules.insert(
            "godot".to_string(),
            vec!["export/"].into_iter().map(String::from).collect(),
        );
        rules.insert(
            "renpy".to_string(),
            vec![
        ...
        rules.insert(
            "mongodb".to_string(),
            vec!["dump/"].into_iter().map(String::from).collect(),
        );
        rules.insert(
            "sqlite".to_string(),
            vec!["*.sqlite3"].into_iter().map(String::from).collect(),
        );
        rules.insert(
            "firebase".to_string(),
            vec![".firebase/"].into_iter().map(String::from).collect(),
        );
        rules.insert(
            "vscode".to_string(),
            vec![".vscode/"].into_iter().map(String::from).collect(),
        );
        rules.insert(
            "idea".to_string(),
            vec![".idea/"].into_iter().map(String::from).collect(),
        );
        rules.insert(
        ...
        rules.insert(
            "atom".to_string(),
            vec!["*.atom"].into_iter().map(String::from).collect(),
        );
        rules.insert(
            "vim".to_string(),
            vec!["*.swp", "*.swo"]
        ...
        rules.insert(
            "scss".to_string(),
            vec![".sass-cache/"].into_iter().map(String::from).collect(),
        );
        rules.insert(
            "sass".to_string(),
            vec![".sass-cache/"].into_iter().map(String::from).collect(),
        ...
        rules.insert(
            "less".to_string(),
            vec![".less-cache/"].into_iter().map(String::from).collect(),
        );
        rules.insert(
            "jupyter".to_string(),
        ...
        rules.insert(
            "terraform".to_string(),
            vec![".terraform/"].into_iter().map(String::from).collect(),
        );
...
fn main() {
    ...
    for arg in args.iter().skip(1) {
        match arg.as_str() {
            "--help" | "-h" => {
                eprintln!("'{}' must be first argument", arg);
                std::process::exit(1);
            }
            "--add" | "-a" | "--remove" | "-r" => {
                ...
                current_action = arg.to_string();
            }
            _ => {
                if current_action.is_empty() {
                    current_action = "--add".to_string();
                }
    ...
    for command in commands {
        for arg in command.args {
            ...
            content = match command.action.as_str() {
                "--add" | "-a" => add_gitignore_rule(content, &arg, &gitignore_rules),
                "--remove" | "-r" => remove_gitignore_rule(content, &arg, &gitignore_rules),
                _ => generate_gitignore_rule(content, &arg, &gitignore_rules),
            };
        }
    }
...
fn show_help() {
    ...
    println!("\n\x1b[1mNote:\x1b[0m");
    println!("  If neither --add or --remove options are used, the .gitignore\n  file will be completely overwritten.");
    println!("\n\x1b[1mRules:\x1b[0m");
